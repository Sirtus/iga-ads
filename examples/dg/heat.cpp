...
auto galerkin_main(int /*argc*/, char* /*argv*/[]) -> void {
    ...
    // Eigen::SparseMatrix<double> ADZ(6, 6);
    ...
    auto mesh = ads::regular_mesh3{xs, ys, ts};
    ...
    // auto tripletList = problem.triplets();
    // for(int i = 1000000; i < 1000030; ++i) {
    //     fmt::print("Triplet{} {}: {}\n", tripletList[i].row(), tripletList[i].col(), tripletList[i].value());
    // }
    fmt::print("prepare data\n");
    problem.prepare_data();
    fmt::print("AFTER prepare_data \n");    

    fmt::print("Solving\n");
    auto xx = solver.solve2(problem);
    // fmt::print("SPODIFJPSDOIFJSDPOFIJSDFPOISDJFPODSIJ {} \n", xx.size());

    // std::ofstream outputFile("");
    // if (outputFile.is_open()) {
    //     outputFile << "Wynik x:\n" << xx << "\n";
    //     // for(int i = 0; i < n; ++i) outputFile << xx[i][1] << "\n";

    //     outputFile.close();
    // } else {
    //     std::cerr << "Nie można otworzyć pliku do zapisu wyniku.\n";
    // }
    ...
    std::ofstream outputFile("");
    if (outputFile.is_open()) {
        outputFile << "MUMPS   EIGEN   DIFF" <<  "\n";
        for(int i = 0; i < n; ++i) {
            outputFile << std::setw(15) << F[i] << " | " << std::setw(15) << xx(i) << " | "<< std::setw(15) << F[i] - xx(i) << "\n";
        } 
...
auto stabilized_main(int argc, char* argv[]) -> void {
    ...
    auto mesh = ads::regular_mesh3{xs, ys, ts};
    auto quad = ads::quadrature3{&mesh, std::max(p_test + 1, 2)};
