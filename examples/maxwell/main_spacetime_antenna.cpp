...
auto parse_args(int argc, char* argv[]) {
    struct {
        int el_x, el_y, el_z, el_t, max_iter, save_problem, from_file, p;
        double toler, T;
    } args{};
    ...
    auto const cli = lyra::help(show_help).description(desc)  //
         ...
         | lyra::opt(args.T, "double").required() ["-T"]["--time"] ("time")
         | lyra::opt(args.p, "integer") ["-p"] ("B-splines degree - default 1")
...
// Example invocation:
// maxwell_spacetime_antenna -x 12 -y 12 -z 12 -t 16 -i 50 -s 0 --toler 0.005 --from_file 0
int main(int argc, char* argv[]) {
    auto const args = parse_args(argc, argv);
    if (args.from_file) maxwell_spacetime_antenna_main_eigen_from_file(args);
    else maxwell_spacetime_antenna_main_eigen(args);
}
